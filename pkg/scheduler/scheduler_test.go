...
// Global token for testing
var TEST_TOKEN string = ""

// Global instance id for testing
var TEST_INSTANCE_ID string = ""
...
func setupTestSchedulerWithVolumeBinding(fakeVolumeBinder *volumebinder.VolumeBinder, stop <-chan struct{}, broadcaster record.EventBroadcaster) (*Scheduler, chan *v1.Binding, chan error) {
	...
	testPVC := v1.PersistentVolumeClaim{ObjectMeta: metav1.ObjectMeta{Name: "testPVC", Tenant: pod.Tenant, Namespace: pod.Namespace, UID: types.UID("testPVC")}}
...
func TestSchedulerWithVolumeBinding(t *testing.T) {
	...
	table := []struct {
	...
	}{
		{
			...
			expectPodBind:      &v1.Binding{ObjectMeta: metav1.ObjectMeta{Name: "foo", Tenant: metav1.TenantSystem, Namespace: "foo-ns", UID: types.UID("foo")}, Target: v1.ObjectReference{Kind: "Node", Name: "machine1"}},
			eventReason:        "Scheduled",
		},
		{
...
func TestRequestToken_SingleRequestWithOneValidHost(t *testing.T) {
	...
	token, err := requestToken(result.SuggestedHost)
	if err != nil {
...
func TestRequestToken_SingleRequestWithOneInvalidHost(t *testing.T) {
	...
	if err == nil {
		t.Errorf("excepted token request fail, but success: %v", err)
	}
}
...
func TestServerCreate_SingleServerRequestWithInvalidHost(t *testing.T) {
	...
	for _, item := range table {
		...
		if err == nil {
			t.Errorf("expected instance create fail but success: %v", err)
		}
	}
}
